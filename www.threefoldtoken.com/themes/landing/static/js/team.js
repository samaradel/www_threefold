    ...
    var render = function(dataset) {
        ...
        if(details) {
          while(true) {
              ...
              var parent = $("<div></div>").addClass("rj-team-member");
              var a = $("<div></div>").addClass('member-photo').click(toggleBio);
              var img = $("<img/>").addClass('rj-team-member-photo-rollover');
              img.prop('src', '../avatars/' + encodeURIComponent(random_details.avatar));
              a.append(img);
              parent.append(a);
              var div = $("<div></div>").addClass("rj-team-member-info-text").css('display', 'none');
                  var imgCol = $("<div></div>").addClass('col-md-3 col-sm-5');
                  var dataCol = $("<div></div>").addClass('col-md-7 col-sm-7"');
                  var close = $('<div></div>').addClass('close-bio').text('x');
                  div.append(imgCol);
                  imgCol.append($("<img/>").prop("src", "../avatars/" + encodeURIComponent(random_details.avatar)));
                  div.append(dataCol);
                  dataCol.append($("<div></div>").addClass('member-name').text(random_details.name));
                  dataCol.append($("<div></div>").addClass('bio-excerpt').text(random_details.description));
                  div.append(close);
                  parent.append(div);
